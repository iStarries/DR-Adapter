...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, transform, split, shot,fold = 4):
        self.split = 'val' if split in ['val', 'test'] else 'trn'
        self.fold = fold          #self.fold = 4
        self.nfolds = 4
        self.nclass = 20
        self.benchmark = 'pascal'
        self.shot = shot
        self.cache = True
        self.img_path = os.path.join('../data/VOCdevkit/VOC2012/JPEGImages/')
        self.ann_path = os.path.join('../data/VOCdevkit/VOC2012/SegmentationClassAug/')
    ...
    def get_weights(self, name, cls, cache=True):
        if self.cache and cache:
            if name not in cache_weights:
                cache_weights[name] = self.load_weights(name)
            weights_dict = cache_weights[name]
        else:
            weights_dict = self.load_weights(name)
    ...
    def build_img_metadata(self):
        ...
        def read_metadata(split, fold_id):
            fold_n_metadata = os.path.join('data_util/splits/pascal/%s/fold%d.txt' % (split, fold_id))
